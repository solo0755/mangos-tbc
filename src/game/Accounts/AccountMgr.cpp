	...
	if (result)
	{
		do
		{
			...
			bool update_sv = LoginDatabase.PExecute(
				"INSERT INTO account(id,username,v,s,email,joindate,  locale) VALUES('%d','%s','%s','%s','%s','%s','%d')",
				id,username.c_str(), v_hex, s_hex, email.c_str(), joindate.c_str(),  locale);
...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password, std::string email) const
{
    ...
    bool update_sv = LoginDatabase.PExecute(
        "INSERT INTO account(username,v,s,email,joindate) VALUES('%s','%s','%s','%s',NOW())",
            username.c_str(), v_hex, s_hex, email.c_str());
...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password, std::string email, uint32 expansion) const
{
	...
	bool update_sv = LoginDatabase.PExecute(
		"INSERT INTO account(username,v,s,email,joindate,expansion) VALUES('%s','%s','%s','%s',NOW(), %u)",
		username.c_str(), v_hex, s_hex, email.c_str(),expansion);
