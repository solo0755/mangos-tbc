...
bool GossipHello_example_creature(Player* pPlayer, Creature* pCreature)
{
	...
	if (pCreature->GetEntry() == sPzxConfig.GetIntDefault("pzx.init.npc", 198601)) {
		...
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"|cFF990066|TInterface\\ICONS\\INV_Misc_Gift_03.blp:30|t|r 免费领取-|cff6247c8职业套装|h|r", GOSSIP_SENDER_MAIN, 400);
	}
	...
	if (pCreature->GetEntry() == sPzxConfig.GetIntDefault("pzx.init.npc", 198601)) {
		if (!pPlayer->IsInCombat() || pPlayer->IsGameMaster()) {//战斗中不显示菜单
			...
			if (sPzxConfig.GetIntDefault("show.additem", 1)) {
				pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_VENDOR, u8"|cFF990066|TInterface\\ICONS\\Mail_GMIcon.blp:30|t|r 输入|cff0070dd物品ID|r获取部分限制物品(151等级以下)", GOSSIP_SENDER_MAIN, 103, u8"在弹框中输入物品ID编号 数量\n 例:|cFF00F0ff需要4个无底包|r，请输入:|cFFF0FF0014156   4|r", 0, true);
			}
