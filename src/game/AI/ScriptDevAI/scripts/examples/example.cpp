	...
	if (uiAction < 1000 && uiAction>100) {
		...
		if (uiAction == 101)
		{
			...
			if (pPlayer->GetMap() && pPlayer->GetMapId() == 580) {
				pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TAXI, u8"传送区域--> 卡雷苟斯平台 |cffff0000 (限团长使用)|h|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 17);
				pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TAXI, u8"传送区域--> 基尔加丹房间 |cffff0000 (限团长使用)|h|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 18);
			}
			...
			case 17://卡雷苟斯的平台
				...
				pPlayer->CLOSE_GOSSIP_MENU();
				pPlayer->TeleportTo(580, 1704.9f, 971.31f, 53.1f, 4.7f);
				break;
			default:
				break;
			}
