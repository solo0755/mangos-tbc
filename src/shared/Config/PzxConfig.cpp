...
bool PzxConfig::SetSource(const std::string& file)
{
	m_filename = file;
...
bool PzxConfig::IsSet(const std::string& name) const
{
	auto const nameLower = boost::algorithm::to_lower_copy(name);
	return m_entries.find(nameLower) != m_entries.cend();
}

const std::string PzxConfig::GetStringDefault(const std::string& name, const std::string& def) const
{
	auto const nameLower = boost::algorithm::to_lower_copy(name);
...
bool PzxConfig::GetBoolDefault(const std::string& name, bool def) const
{
...
int32 PzxConfig::GetIntDefault(const std::string& name, int32 def) const
{
...
float PzxConfig::GetFloatDefault(const std::string& name, float def) const
{
	auto const value = GetStringDefault(name, std::to_string(def));
